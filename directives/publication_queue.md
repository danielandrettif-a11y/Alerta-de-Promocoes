...
Prepare Mercado Livre, Amazon and Shopee offers for review. Affiliate links can be supplied
...
- Mercado Livre accepts only HTTPS links on the exact `meli.la` hostname.
- Shopee accepts only HTTPS links on the exact `s.shopee.com.br` hostname.
- Amazon accepts only `amazon.com.br`/`www.amazon.com.br` links with an
...
- The extension must reserve `ready` for items whose current product price was
  read on the product page. Missing price proof keeps the item in review.
- Store `priceVerification.regularPrice`, `finalPrice`, `verifiedAt`, `source`
  and `productId` with the queue item.

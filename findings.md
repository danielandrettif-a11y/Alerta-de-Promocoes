...
## Alinhamento e Diagnóstico da Aba Amazon (Fila & Scraping)
- **Falta do Botão de Fila na UI da Amazon**: No `public/index.html`, o contêiner `panel-amazon` possui apenas os botões "Atualizar Amazon" e "Enviar agora". Os botões `#btn-queue-amazon` e `#txt-queue-count-amazon` não existiam no HTML da aba Amazon, ao contrário dos contêineres do Mercado Livre e Shopee.
- **Formato das Ofertas e Array Vazio em Scraping**: O arquivo `amazon_deals_report.json` estava salvo com `deals: []` (0 ofertas) devido à mudança nos seletores de grid da Amazon Brasil (`https://www.amazon.com.br/deals`). É necessário atualizar os seletores do Puppeteer em `execution/amazon_deals.js` para varrer múltiplas categorias nobres e extrair informações completas de produto, preço, desconto, frete Prime e cupons de clipe com a tag de afiliado.
